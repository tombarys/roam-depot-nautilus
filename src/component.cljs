...
(defn populate-events
  ...
  [block-uid plan-from-time]
  (let [text->events (-> (mapv parse-row-params (get-children-strings block-uid))
                         (add-start-after))
...
(defn main [{:keys [block-uid]} & args]
  ...
  (let [dimensions {:width (if mobile? mob-width desk-width)
        ...
        events-state (r/atom (populate-events block-uid plan-from-time))]
    (println?debug "Dimensions-atom: " dimensions)
    (println?debug "Argumenty roam/renderu: " args)
    [:div
     (if-not (nil? @events-state)
       [show-events events-state daily-page-atom? show-done-state page-title dimensions]
       (reset! events-state (populate-events block-uid plan-from-time)))
     [:div
